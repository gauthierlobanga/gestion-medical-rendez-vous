...
class ListPermissions extends ListRecords
{
    ...
    public function getTabs(): array
    {
        ...
        $thisWeek = Permission::query()->where('created_at', '>=', now()->subWeek())?->count();
        $thisMonth = Permission::query()->where('created_at', '>=', now()->subMonth())?->count();
        $thisYear = Permission::query()->where('created_at', '>=', now()->subYear())?->count();
        $thisQuarter = Permission::query()->where('created_at', '>=', now()->subQuarter())?->count();
        $thisSemester = Permission::query()->where('created_at', '>=', now()->subMonths(6))?->count();
