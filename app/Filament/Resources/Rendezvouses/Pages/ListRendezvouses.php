...
class ListRendezvouses extends ListRecords
{
    ...
    public function getTabs(): array
    {
        ...
        $thisWeek = Rendezvous::query()->where('created_at', '>=', now()->subWeek())?->count();
        $thisMonth = Rendezvous::query()->where('created_at', '>=', now()->subMonth())?->count();
        $thisYear = Rendezvous::query()->where('created_at', '>=', now()->subYear())?->count();
        $thisQuarter = Rendezvous::query()->where('created_at', '>=', now()->subQuarter())?->count();
        $thisSemester = Rendezvous::query()->where('created_at', '>=', now()->subMonths(6))?->count();
