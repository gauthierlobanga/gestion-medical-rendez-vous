...
class ListUsers extends ListRecords
{
    ...
    public function getTabs(): array
    {
        ...
        $thisWeek = User::query()->where('created_at', '>=', now()->subWeek())?->count();
        $thisMonth = User::query()->where('created_at', '>=', now()->subMonth())?->count();
        $thisYear = User::query()->where('created_at', '>=', now()->subYear())?->count();
        $thisQuarter = User::query()->where('created_at', '>=', now()->subQuarter())?->count();
        $thisSemester = User::query()->where('created_at', '>=', now()->subMonths(6))?->count();
